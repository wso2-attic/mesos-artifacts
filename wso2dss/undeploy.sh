...
function undeploy_product() {
  undeploy wso2dss-default
  undeploy wso2dss-worker
  undeploy wso2dss-manager
  undeploy mysql-dss-db
}
...
function main() {
  ...
  if [[ $full_purge == true ]]; then
    echo "Purging WSO2 DSS deployment..."
    full_purge
  else
    echo "Undeploying WSO2 DSS product..."
    undeploy_product
  fi
}
main "$@"
