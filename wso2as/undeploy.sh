...
function undeploy_product() {
  undeploy wso2as-default
  undeploy wso2as-worker
  undeploy wso2as-manager
  undeploy mysql-as-db
}
...
function main() {
  ...
  if [[ $full_purge == true ]]; then
    echo "Purging WSO2 AS deployment..."
    full_purge
  else
    echo "Undeploying WSO2 AS product..."
    undeploy_product
  fi
}
main "$@"
