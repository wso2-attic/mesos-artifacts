...
function replace_local_member_host_with_ip {
    ...
    if [[ $? == 0 ]];
        then
        echo "successfully updated localMemberHost with host ip address $local_ip and localMemberPort with port $PORT0"
    else
        echo "error occurred in updating localMemberHost with local ip address $host_ip and localMemberPort with port $PORT0"
    fi
}
...
echo "Starting ${WSO2_SERVER} at ${CARBON_HOME}..."
