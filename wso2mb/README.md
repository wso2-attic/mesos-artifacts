...
To deploy a WSO2 Message Broker on Mesos DC/OS, follow the below steps:

#### 1. Build WSO2 Message Broker Docker Image

To manage configurations and artifacts when building Docker images, WSO2 recommends to use [`wso2/puppet-modules`](https://github.com/wso2/puppet-modules) as the provisioning method. A specific data set for Mesos platform is available in WSO2 Puppet Modules. It's possible to use this data set to build Dockerfiles for WSO2 Message Broker for Mesos with minimum configuration changes.

Building WSO2 Message Broker Docker image using Puppet for Mesos:
  ...
  4. Copy the [`mysql-connector-java-5.1.36-bin.jar`](https://downloads.mysql.com/archives/get/file/mysql-connector-java-5.1.36.zip) to `PUPPET_HOME/modules/wso2mb/files/configs/repository/components/lib` location.
  5. Get the WSO2 Message Broker product distribution (3.0.0 or 3.1.0) and copy it to `PUPPET_HOME/modules/wso2mb/files` location.
  6. Set the environment variable `PUPPET_HOME` pointing to location of the puppet modules in local machine.
  7. Navigate to `wso2mb` directory in the Dockerfiles repository; `DOCKERFILES_HOME/wso2mb`.
  8. Build the Dockerfile with the following command:
    ...
    **`./build.sh -v <version> -s mesos -r puppet`**
    ...
    This will build the default profile of WSO2 Message Broker for Mesos platform, using configuration specified in Puppet.
...
**`./load-images.sh -u centos -p wso2mb -k /home/ssh_key.pem`**
...
##### 3. Deploy WSO2 Message Broker on Mesos DC/OS
  1. Navigate to `wso2mb` directory in mesos-artifacts repository; `MESOS_HOME/wso2mb` location.
  2. Run the deploy.sh script:
  
    Note that by default MB `3.1.0` Docker image is used in the Marathon application. Update the docker image version in MB Marathon application to use a different version.

    **`./deploy.sh`**
    
    This will deploy following Marathon applications in Mesos DC/OS, using the images available in Mesos slave nodes, and notify once the intended Marathon application `wso2mb-default` starts running on the container.
       * Marathon load balancer
       * WSO2 Governance Registry database
       * WSO2 User Management database
       * WSO2 Message Broker Configuration Registry database
       * WSO2 Message Broker default profile
       
#### 4. Login to WSO2 Message Broker Management Console
  ...
  2. Login to the Carbon Management Console URL using `https://marathon-lb.marathon.mesos:10123/carbon/`.
 
#### 5. Undeploy WSO2 Message Broker from Mesos DC/OS
  1. Navigate to `wso2mb` directory in mesos-artifacts repository; `MESOS_HOME/wso2mb` location.
  2. Run the `undeploy.sh` script:

    **`./undeploy.sh`**

    This will undeploy the WSO2 Message Broker and its Configuration Registry database(`mysql-mb-db`) Marathon applications.
...
For more detailed instructions on deploying WSO2 Message Broker on Mesos DC/OS, please refer the wiki links under the Documentation section below.
