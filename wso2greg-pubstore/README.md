# Mesos Artifacts for WSO2 Governance Registry Publisher Store
...
To deploy a WSO2 Governance Registry Publisher Store on Mesos DC/OS, follow the below steps:

#### 1. Build WSO2 Governance Registry Publisher Store Docker Image

To manage configurations and artifacts when building Docker images, WSO2 recommends to use [`wso2/puppet-modules`](https://github.com/wso2/puppet-modules) as the provisioning method. A specific data set for Mesos platform is available in WSO2 Puppet Modules. It's possible to use this data set to build Dockerfiles for WSO2 Governance Registry Publisher Store for Mesos with minimum configuration changes.

Building WSO2 Governance Registry Publisher Store Docker image using Puppet for Mesos:
  ...
  4. Copy the [`mysql-connector-java-5.1.36-bin.jar`](https://downloads.mysql.com/archives/get/file/mysql-connector-java-5.1.36.zip) to `PUPPET_HOME/modules/wso2greg_pubstore/files/configs/repository/components/lib` location.
  5. Get the WSO2 Governance Registry Publisher Store product distribution (2.1.0 or 2.2.0) and copy it to `PUPPET_HOME/modules/wso2greg_pubstore/files` location.
  6. Set the environment variable `PUPPET_HOME` pointing to location of the puppet modules in local machine.
  7. Navigate to `wso2greg-pubstore` directory in the Dockerfiles repository; `DOCKERFILES_HOME/wso2greg-pubstore`.
...
**`./load-images.sh -u centos -p wso2greg-pubstore -k /home/ssh_key.pem`**
...
##### 3. Deploy WSO2 Governance Registry Publisher Store on Mesos DC/OS
  1. Navigate to `wso2greg-pubstore` directory in mesos-artifacts repository; `MESOS_HOME/wso2greg-pubstore` location.
  2. Run the deploy.sh script:

    **`./deploy.sh`**
    
    This will deploy following Marathon applications in Mesos DC/OS, using the images available in Mesos slave nodes, and notify once the intended Marathon application `wso2greg-pubstore-default` starts running on the container.
       * Marathon load balancer
       * WSO2 Governance Registry database
       * WSO2 User Management database
       * WSO2 Governance Registry Publisher Store Configuration Registry database
       * WSO2 Governance Registry Publisher Store default profile

#### 4. Login to WSO2 Governance Registry Publisher Store Management Console
  ...
  2. Login to the Carbon Management Console URL using `https://marathon-lb.marathon.mesos:10134/carbon/`.
 
#### 5. Undeploy WSO2 Governance Registry Publisher Store from Mesos DC/OS
  1. Navigate to `wso2greg-pubstore` directory in mesos-artifacts repository; `MESOS_HOME/wso2greg-pubstore` location.
  2. Run the `undeploy.sh` script:

    **`./undeploy.sh`**

    This will undeploy the WSO2 Governance Registry Publisher Store and its Configuration Registry databases(`mysql-greg-db` and `mysql-greg-apim-db`) Marathon applications.
...
For more detailed instructions on deploying WSO2 Governance Registry Publisher Store on Mesos DC/OS, please refer the wiki links under the Documentation section below.
