...
To deploy a WSO2 Complex Event Processor on Mesos DC/OS, follow the below steps:

#### 1. Build WSO2 Complex Event Processor Docker Image

To manage configurations and artifacts when building Docker images, WSO2 recommends to use [`wso2/puppet-modules`](https://github.com/wso2/puppet-modules) as the provisioning method. A specific data set for Mesos platform is available in WSO2 Puppet Modules. It's possible to use this data set to build Dockerfiles for WSO2 Complex Event Processor for Mesos with minimum configuration changes.

Building WSO2 Complex Event Processor Docker image using Puppet for Mesos:
  ...
  4. Copy the [`mysql-connector-java-5.1.36-bin.jar`](https://downloads.mysql.com/archives/get/file/mysql-connector-java-5.1.36.zip) to `PUPPET_HOME/modules/wso2cep/files/configs/repository/components/lib` location.
  5. Get the WSO2 Complex Event Processor product distribution (4.0.0) and copy it to `PUPPET_HOME/modules/wso2cep/files` location.
  6. Set the environment variable `PUPPET_HOME` pointing to location of the puppet modules in local machine.
  7. Navigate to `wso2cep` directory in the Dockerfiles repository; `DOCKERFILES_HOME/wso2cep`.
  8. Build the Dockerfile with the following command:
    ...
    **`./build.sh -v <version> -s mesos -r puppet`**
    ...
    This will build the default profile of WSO2 Complex Event Processor for Mesos platform, using configuration specified in Puppet.
...
**`./load-images.sh -u centos -p wso2cep -k /home/ssh_key.pem`**
...
##### 3. Deploy WSO2 Complex Event Processor on Mesos DC/OS
  1. Navigate to `wso2cep` directory in mesos-artifacts repository; `MESOS_HOME/wso2cep` location.
  2. Run the deploy.sh script:

    **`./deploy.sh`**
    
    This will deploy following Marathon applications in Mesos DC/OS, using the images available in Mesos slave nodes, and notify once the intended Marathon application `wso2cep-default` starts running on the container.
       * Marathon load balancer
       * WSO2 Governance Registry database
       * WSO2 User Management database
       * WSO2 Complex Event Processor Configuration Registry database
       * WSO2 Complex Event Processor default profile
       
#### 4. Login to WSO2 Complex Event Processor Management Console
  ...
  2. Login to the Carbon Management Console URL using `https://marathon-lb.marathon.mesos:10053/carbon/`.
 
#### 5. Undeploy WSO2 Complex Event Processor from Mesos DC/OS
  1. Navigate to `wso2cep` directory in mesos-artifacts repository; `MESOS_HOME/wso2cep` location.
  2. Run the `undeploy.sh` script:

    **`./undeploy.sh`**

    This will undeploy the WSO2 Complex Event Processor and its Configuration Registry database(`mysql-cep-db`) Marathon applications.
...
For more detailed instructions on deploying WSO2 Complex Event Processor on Mesos DC/OS, please refer the wiki links under the Documentation section below.
