...
function undeploy_product() {
  undeploy wso2esb-default
  undeploy wso2esb-worker
  undeploy wso2esb-manager
  undeploy mysql-esb-db
}
...
function main() {
  ...
  if [[ $full_purge == true ]]; then
    echo "Purging WSO2 ESB deployment..."
    full_purge
  else
    echo "Undeploying WSO2 ESB product..."
    undeploy_product
  fi
}
main "$@"
